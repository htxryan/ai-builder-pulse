...
/** Derive the UTC `YYYY-MM-DD` runDate for `now`. */
...
/** Whether `s` matches the `YYYY-MM-DD` runDate shape. */
export function isValidRunDate(s: string): boolean {
  return RUN_DATE_RE.test(s);
}

/** Date `hours` before midnight-UTC of `runDate`. Throws on malformed input. */
